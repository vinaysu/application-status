...
function HigherOfficer() {
  ...
  function handleSubmit() {
    ...
    const application = {
      ...
      status: [`Submitted by HO on ${new Date().toLocaleString()}`],
      printStatus: true,
  ...
  function handleReject(index) {
    ...
    updatedList[index] = { ...updatedList[index], HOreject: true, HOverifyButton: true, status: [`Rejected  by HO on ${new Date().toLocaleString()}`, ...updatedList[index].status] };
    setList(updatedList);
  ...
  function handleApprove(index) {
    ...
    updatedList[index] = {
      ...updatedList[index],
      HOVerify: true, HOrejectButton: true,
      status: [`Approved  by HO on ${new Date().toLocaleString()}`, ...updatedList[index].status],
      printStatus: false
    };
  ...
  return (
    <div className={styles.container}>
...

        {
          list.map((ele, index) =>
            <div className={styles.applications}>
              ...
              <Button onClick={() => handleReject(index)} className={styles.item} disabled={ele.HOrejectButton}>
                {ele.HOreject ? <span style={{ color: 'red' }}>rejected</span> : 'reject'}

              </Button>
              <Button onClick={() => handleApprove(index)} className={styles.item} disabled={ele.HOverifyButton} >
                {ele.HOVerify ? <span style={{ color: 'green' }}>Approved</span> : 'Approve'}
      ...
      <div className={styles.right} >
        ...
        <input className={styles.input} value={enteredData.title} name='title' onChange={handleChange} placeholder='ENTER THE TITLE' ></input>
